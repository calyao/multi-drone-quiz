...
def esdf(M, N, obstacle_list):
    ...
    queue = collections.deque(
        [(tuple(l), tuple(l)) for l in obstacle_list])  # queue values must contain the target coordinate and BFS root
    ...
    while queue:
        ...
        if stored_dist == -1 or stored_dist > cur_dist:  # if this isn't true, nothing happens this round
            ...
            for i in range(4):
                ...
                # bounds check
                if 0 <= adjx < M and 0 <= adjy < N:
                    if test_space not in seen or grid[adjx][adjy] == -1 or grid[adjx][adjy] > math.dist(test_space, root):
